...
Tiger1 PCB용입니다. (King Tiger용은 구현이 상이하며 별도 PCB와 코드)
...
#### 포 마운트 서보 모터 (각도 조절)
- **신호선**: GPIO 19
- **전원**: 5V (4.2V 동작 모터이므로 과열 방지를 위해 대기 시 자동 비활성화)
- **접지**: 공통 접지
- **기능**: D-PAD 상하로 포 마운트 각도 조절 (마지막 조작 후 2초간 입력이 없으면 서보 토크 자동 해제)

#### 포신 서보 모터 (당기기)
- **신호선**: GPIO 18
- **전원**: 5V
- **접지**: 공통 접지
- **기능**: B 버튼으로 포신 당기기 (발사 시퀀스 완료 500ms 후 서보 토크 자동 해제)
