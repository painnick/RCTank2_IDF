...
esp_err_t rctank_motor_init(void)
{
    ...
    ESP_RETURN_ON_ERROR(ret, TAG, "mcpwm_new_timer");
    ...
    ret = mcpwm_operator_connect_timer(left_op, timer);
    ESP_RETURN_ON_ERROR(ret, TAG, "connect timer left");
    ret = mcpwm_operator_connect_timer(right_op, timer);
    ESP_RETURN_ON_ERROR(ret, TAG, "connect timer right");
    ret = mcpwm_operator_connect_timer(turret_op, timer);
    ...
    ESP_RETURN_ON_ERROR(ret, TAG, "timer_enable");
    ret = mcpwm_timer_start_stop(timer, MCPWM_TIMER_START_NO_STOP);
    ESP_RETURN_ON_ERROR(ret, TAG, "timer_start");
