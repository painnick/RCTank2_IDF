...
void dumpGamepad(ControllerPtr ctl) {
    Console.printf(
        "idx=%d, %c%c%c%c %s %s %c%c%c%c dpad: 0x%02x, buttons: 0x%04x, axis L: %4d, %4d, axis R: %4d, %4d, brake: %4d, throttle: %4d, "
        ...
        ctl->miscSelect() ? "SEL" : "---",
        ctl->miscStart() ? "STA" : "---",
        ctl->dpad() == DPAD_LEFT ? 'L' : '-',
        ctl->dpad() == DPAD_RIGHT ? 'R' : '-',
        ctl->dpad() == DPAD_UP ? 'U' : '-',
        ctl->dpad() == DPAD_DOWN ? 'D' : '-',
        ctl->dpad(),         // D-pad
